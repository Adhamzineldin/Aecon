...
def get_locations(request):
    ...
    for item in proj_loc:
        ...
        if not item.speed_limit:
            project_lst.append(
                {'id': project.id, 'name': project.project_no, "start_date": item.startDate, "end_date": item.endDate})
        else:
            project_lst.append(
                {'id': project.id, 'name': project.project_no, 'speed_limit': item.speed_limit, "start_date": item.startDate,
    ...
    for arm in arms:
        arms_lst.append(
            {'id': arm.id, 'name': arm.name, 'Fullname': arm.display_name, 'lat': arm.lat, 'lon': arm.lon, 'project': arm.project.id, 'location': arm.location.id})
...
def get_API_classes(request):
    if request.method == "POST":
        return JsonResponse(
            {"status": "ok", "data": VivacityAPI.objects.using(DB_NAME).get(name="CRT").classes_as_html_menu_list()})
    return JsonResponse({"status": "failed", "message": "incorrect request method"})
...
def reset_password(request):
    if request.method == "POST":
        ...
        if "Email" in request.POST:
            ...
            return JsonResponse(
                {"status": "success", "user_id": user.id, "message": "Please check your email for reset password link"})
        ...
        if user.username in jsondata.keys():
            if str(survey_type_flag) in jsondata[user.username]:
                ...
                upload_to_db_thread = threading.Thread(target=upload_data_to_db_thread,
                                                       args=(user, jsondata[user.username][str(survey_type_flag)]['files'], int(survey_type_flag)))
...
def aggregated_data(request):
    ...
    if speed_flg == 'false' and public_view == 'false':
        ...
        projects = list(
            ProjectLocations.objects.using(DB_NAME).filter(location=loc).order_by('startDate').values_list('project',
                                                                                                           flat=True))
                ...
                for t in time:
                    for item in data:
                        if item['day'] == d and item['direction__order'] == dir and item['timeval'] == t:
                            value.append({"time": t, 'avg': round(item['avg']), 'perc_85th': round(item['perc_85th']),
...
def upload_data_to_db_thread(user, uploaded_files_names, survey_type_flag):
    ...
    if failed_file_errors:
        ...
        content += "<p>More Info: " + \
            ",<br> ".join(
                f"<br>File Name:<br>&emsp;&emsp;{file}<br>File Error:<br>&emsp;&emsp;{error}" for file, error in failed_file_errors.items()) + "<br></p>"
...
def upload_site_observations_to_db(data_filename, site_filename, data_type_flag, log):
    ...
    if data_type_flag == 11:
        ...
        class_lst = {1: 43107, 2: 43108, 3: 43109, 4: 43110, 5: 43111, 6: 43112, 7: 43117, 8: 43118, 9: 43119, 10: 43120,
                     11: 43121, 12: 43122, 13: 43122}
    else:
        class_lst = {1: 43103, 2: 43104, 3: 43104, 4: 43105, 5: 43105, 6: 43106, 7: 43106, 8: 43106, 9: 43106, 10: 43106,
    ...
    if request.method == 'GET':
        ...
        for pro in projects:
            temp = {
                'id': pro.id,
                'name': pro.project_no,
                'sites': [{'id': loc.id, 'name': loc.name,'obs_type':loc.observationType_id} for loc in pro.location_set.all()],
...
def jtc_download_thread(user, project_ids, location_ids, root_url, token, is_turning_count):
    try:
        ...
        extracted_value = ['project__project_no', 'project__name', 'location__api_identifier',
                                                                                                        'location__lat', 'location__lon', 'start_time', 'end_time', 'peak_hour', 'origin_arm__name', 'destination_arm__name', 'obsClass__order', 'count', 'pcu']
        
        df_columns = ['Project No', 'Project Name', 'Site Number', 'Latitude', 'Longitude',
                                                   'Start_Time', 'End_Time', 'Peak Hour', 'Origin_Arm', 'Destination_Arm', 'Vechicle Class', 'Vechile Count', 'PCU Value']
        if is_turning_count:
        ...
        for proj in projects:
            ...
            for loc in locations:
                jtc_data = Jtc_Data.objects.filter(project__id__in=project_ids, location=loc).values_list(*extracted_value)

                jtc_df = pd.DataFrame.from_records(jtc_data, columns=df_columns)
                
                if is_turning_count:
                    jtc_df['Start_Date'] = pd.to_datetime(jtc_df.Start_Date.astype(str) + ' ' + jtc_df.Start_Time.astype(str)) 
                ...
                arms_data = Arms.objects.filter(location=loc, project=proj).extra(
                    select={'Site_No': site_no, 'Project_No': f"'{project_no}'"}).values_list('name', 'lat', 'lon', 'Site_No', 'Project_No', 'display_name')

                arms_df = pd.DataFrame.from_records(arms_data, columns=[
                                                    'Name', 'Lat', 'Long', 'Site_No', 'Project_No', 'Display_Name'])

                site_data = ProjectLocations.objects.filter(project=proj, location=loc).extra(select={'Site_ID': F"'{site_id}'", 'Project_No': f"'{project_no}'", 'Project_Name': f"'{project_name}'", 'Site_No': f"'{site_no}'", 'Site_Name': f"'{site_name}'",
                                                                                                      'Survey_Type': f"'{survey_type}'", 'Lat': loc.lat, 'Long': loc.lon}).values_list('Site_ID', 'Project_No', 'Project_Name', 'Site_No', 'Site_Name', 'Survey_Type', 'Lat', 'Long', 'startDate', 'endDate')

                site_df = pd.DataFrame.from_records(site_data, columns=[
                                                    'Site_ID', 'Project_No', 'Project_Name', 'Site_No', 'Site_Name', 'Survey_Type', 'Lat', 'Long', 'startDate', 'endDate'])
...
def get_jtc_data(request):
    try:
        ...
        # set_cookie("Location", ids, max_age = None, expires = None)
        data = Jtc_Data.objects.using(DB_NAME).filter(location_id__in=locationid, project_id__in=projectid, count__gt=0).values_list(
            'location_id', 'project_id', 'origin_arm_id', 'destination_arm_id', 'obsClass_id', 'start_time', 'end_time', 'count', 'pcu')
...
def add_update_jtc_Observation(request):
    try:
        ...
        ObservationMessages, created = Messages.objects.using(DB_NAME).update_or_create(location=location_instance, defaults={
            'location': location_instance, 'text': Observation, 'user':  request.user, 'date': datetime.datetime.now().date()})
...
def addUpdateObservation(request):
    try:
        ...
        location_instance = Location.objects.get(id=loc_id)
        ObservationMessages, created = Messages.objects.update_or_create(location=location_instance, project=project_id, defaults={
            'location': location_instance, 'text': Observation, 'user':  request.user, 'date': datetime.datetime.now().date(), 'project':project_id})
        response = {'status': 'success', 'data': created}
    except Exception as e:
        response = {'status': 'error', 'message': str(e)}
    return JsonResponse(response)
